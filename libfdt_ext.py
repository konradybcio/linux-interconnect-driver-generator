...
def fdt_as_array(self, fmt) -> List[Any]:
...
def fdt_as_int32_array(self) -> List[int]:
    return self.as_array('l')


def fdt_as_uint32_array(self) -> List[int]:
    return self.as_array('L')


def fdt_as_stringlist(self) -> List[str]:
    if self[-1] != 0:
...
def fdt_getprop_or_none(self, nodeoffset, prop_name) -> Optional[Property]:
